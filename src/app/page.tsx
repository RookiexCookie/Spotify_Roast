...
const themes = {
  neon: {
    ...
    cardHover:
      "hover:border-green-400/50 hover:shadow-[0_0_30px_rgba(74,222,128,0.3)]",
  ...
  cyberpunk: {
    ...
    cardHover:
      "hover:border-yellow-400/50 hover:shadow-[0_0_30px_rgba(250,204,21,0.3)]",
  ...
  retro: {
    ...
    cardHover:
      "hover:border-blue-400/50 hover:shadow-[0_0_30px_rgba(96,165,250,0.3)]",
    button:
  ...
  minimal: {
    ...
    cardHover:
      "hover:border-white/40 hover:shadow-[0_0_30px_rgba(255,255,255,0.15)]",
  ...
  return (
    <div className="perspective-container" style={{ perspective: "1000px" }}>
      ...
      >
        ...
        {/* 2. DARK OVERLAY */}
        <div
          className={`absolute inset-0 z-10 bg-gradient-to-t from-black via-black/40 to-transparent transition-all duration-500 ${isRevealed ? "bg-black/80 opacity-100" : "opacity-80"}`}
        />
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-end p-8">
          {/* MOVING CONTAINER */}
          <div
            className={`transform transition-transform duration-500 ease-out ${isRevealed ? "-translate-y-4" : "translate-y-[10px]"}`}
          >
            {/* LABEL */}
            <span
              className={`text-[10px] font-extrabold ${theme.accent} uppercase tracking-[0.2em] mb-2 block transition-opacity duration-300 ${isRevealed ? "opacity-100" : "opacity-0"}`}
            >
              DETECTED
            </span>
            ...
            <p
              className={`md:hidden text-xs ${theme.accent} animate-pulse mb-2 ${isRevealed ? "hidden" : "block"}`}
            >
              Tap to see roast...
            </p>
            ...
            <div
              className={`overflow-hidden transition-all duration-500 ${isRevealed ? "h-auto opacity-100" : "h-0 opacity-0"}`}
            >
              <div
                className={`w-10 h-1 bg-gradient-to-r ${theme.primary} mb-4 rounded-full`}
              />
        ...
        {/* BORDER GLOW */}
        <div
          className={`absolute inset-0 border-2 border-transparent transition-all duration-300 rounded-3xl pointer-events-none ${isRevealed ? theme.cardHover.replace("hover:", "") : ""}`}
        />
      </div>
    </div>
  );
...
const ScoreSkeleton = ({ theme }: { theme: any }) => (
  <div className="text-center mb-20 relative flex flex-col items-center animate-pulse">
    ...
    <div
      className={`h-40 w-64 rounded-xl animate-shimmer ${theme.cardBorder} border-white/5`}
    ></div>
...
const SkeletonCard = ({ theme }: { theme: any }) => (
  <div
    className={`relative h-[450px] w-full bg-white/5 backdrop-blur-md border ${theme.cardBorder} rounded-3xl overflow-hidden shadow-2xl`}
  >
...
const Footer = ({ theme }: { theme: any }) => (
  <footer className="w-full py-12 flex flex-col items-center gap-4 relative z-10 border-t border-white/5 mt-20">
    <a
      href="https://github.com/RookiexCookie/Spotify_Roast"
      target="_blank"
      rel="noopener noreferrer"
      className={`group flex items-center gap-2 transition-all duration-300 hover:scale-105`}
    >
      <span className="text-gray-500 text-sm font-medium">Built by</span>
      <span
        className={`font-black tracking-tight text-lg ${theme.accent} group-hover:text-white transition-colors`}
      >
        RookiexCookie
      </span>
      <svg
        ...
        strokeWidth={2}
        stroke="currentColor"
        className={`w-4 h-4 text-gray-600 group-hover:${theme.accent.replace("text-", "text-")} transition-colors`}
      >
        <path
          strokeLinecap="round"
...
export default function Home() {
  ...
  const handleRoast = async () => {
    ...
    try {
      ...
      if (mode === "user") {
        ...
        if (!artistsRes.ok || !tracksRes.ok) {
          ...
          if (artistsRes.status === 401 || tracksRes.status === 401) {
            alert(
              "Spotify Session Expired. Please sign out and sign in again.",
            );
            return;
          }
        ...
        if (!playlistIdMatch) {
          alert(
            "Invalid Spotify Playlist URL. It should look like: open.spotify.com/playlist/...",
          );
          setLoading(false);
          return;
        }
      ...
      console.error(error);
      alert(
        "System Failure: The AI refuses to perceive this level of bad taste. (Check Console for details)",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main
      className={`min-h-screen ${theme.bg} ${theme.text} flex flex-col items-center p-8 relative overflow-hidden transition-colors duration-500`}
    >
      ...
      <div className="absolute top-6 right-6 z-50">
        <button
          ...
          className={`p-3 rounded-full bg-white/10 backdrop-blur-md border ${theme.cardBorder} transition hover:scale-110`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
          ...
          <div className="absolute right-0 mt-2 w-48 bg-white/10 backdrop-blur-xl rounded-xl border border-white/20 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2">
            {Object.entries(themes).map(([key, t]) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 hover:bg-white/10 transition flex items-center gap-2 ${currentTheme === key ? theme.accent : ""}`}
              >
                <div
                  className={`w-3 h-3 rounded-full bg-gradient-to-r ${t.primary}`}
                ></div>
                {t.name}
              </button>
            ))}
      ...
      <div className="relative z-10 max-w-5xl w-full flex flex-col items-center">
        ...
        <div className="text-center mt-10 mb-8">
          <h1
            className={`text-7xl font-black mb-4 tracking-tighter bg-gradient-to-r ${theme.primary} text-transparent bg-clip-text drop-shadow-sm`}
          >
            ROAST MY SPOTIFY
          </h1>
          <p className={`${theme.text} opacity-80 text-xl`}>
            Dare to let an AI judge your horrible music taste?
          </p>
        </div>
        ...
        {!session ? (
          <button
            ...
            className={`group relative px-8 py-4 rounded-full font-bold text-lg transition hover:scale-105 ${theme.button} shadow-lg mt-8`}
          >
            ...
            >
              ...
              <div className="flex flex-col text-left">
                <span className="text-sm font-medium opacity-80 group-hover:text-red-300">
                  Logged in as{" "}
                  <span className={theme.accent}>{session.user?.name}</span>
                </span>
                <span className="text-[10px] text-red-400 opacity-0 h-0 group-hover:h-auto group-hover:opacity-100 transition-all">
                  Click to Sign Out
                </span>
              </div>
            </button>
            ...
            {loading ? (
              ...
              <div className="text-center mb-16 relative animate-in fade-in slide-in-from-bottom-8 duration-700">
                <div className="inline-block relative">
                  <h2
                    className={`text-[10rem] leading-none font-black text-transparent bg-clip-text bg-gradient-to-b ${theme.score} drop-shadow-2xl`}
                  >
                    {score}
                  </h2>
                  <span
                    className={`absolute top-4 -right-8 text-4xl font-bold ${theme.accent} rotate-12 opacity-80`}
                  >
                    /100
                  </span>
                </div>
                <p
                  className={`${theme.accent} uppercase tracking-[0.3em] mt-4 font-bold text-lg`}
                >
